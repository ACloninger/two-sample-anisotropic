...
for icol=1:ncol
    ...
    for irun=1:nrun
        ...
        [X,Y]=gaussianmixture3d1(ntest,delta,epsx);
...
[xx,~]=gaussianmixture3d1(ntest*2,delta,epsx);
...
figure(31),clf;
scatter3(xx(:,1),xx(:,2),xx(:,3),80,w1,'o','filled')
colorbar();grid on;
...
figure(32),clf;
scatter3(xx(:,1),xx(:,2),xx(:,3),80,w2,'o','filled')
colorbar();grid on;
title('witness ak l2')
...
figure(33),clf;
scatter3(xx(:,1),xx(:,2),xx(:,3),80,w3,'o','filled')
colorbar();grid on;
...
%%%%%%%%%%%%%
function [x,y,gmp,gmq]=gaussianmixture3d1(n,Delta,epsX)
...
b=1/3;
S=zeros(3,3,k);
S(:,:,1)=diag([b.^2,epsX.^2,epsX.^2]);
S(:,:,2)=diag([epsX.^2,b.^2,epsX.^2]);
S(:,:,3)=diag([epsX.^2,epsX.^2,b.^2]);
...
mu1=mu;
S1=S;
gmp=gmdistribution(mu1,S1,p);

mu2=mu;
mu2(1,2)=mu2(1,2)+Delta;
mu2(2,3)=mu2(2,3)+Delta;
mu2(3,1)=mu2(3,1)+Delta;

S2=S;
gmq=gmdistribution(mu2,S2,p);
...
function [R,gmR]=generate_ref_local_pca_prefix(nR,funcXY,sig2g)
...
[~,~,gmp,gmq]=gaussianmixture3d1(npool,0,sqrt(sig2g));

labelR=gmp.cluster(R);
SR=zeros(dim,dim,nR);
for iR=1:nR
    SR(:,:,iR)=gmp.Sigma(:,:,labelR(iR));
end
